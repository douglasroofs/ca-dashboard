...
async function main() {
  ...
  for (const r of board.rows) {
    ...
    if (!cur) {
      ledger.customers[id] = { first_seen: evidence, last_seen: today, customer: r.customer, rep: r.rep, pm: r.pm, number: r.number, source: r.source };
      added.push(`${r.customer} (${r.source}, since ${evidence})`);
    } else {
      if (evidence < cur.first_seen) cur.first_seen = evidence;
      Object.assign(cur, { last_seen: today, customer: r.customer, rep: r.rep, pm: r.pm, number: r.number, source: r.source });
    }
  }
