...
Promise.all([
  ...
  fetch('/api/doors',{cache:'no-store'}).then(function(r){return r.json()}).catch(function(){return {reps:[],total:0}})
]).then(function(res){
...
'<tfoot><tr><td></td><td>Total (office)</td><td class="num">'+(dr.total!=null?dr.total:'-')+'</td><td class="num">'+(ca.total!=null?ca.total:tC)+'</td><td class="num">'+fmt(rev.company.approved_amount)+'</td><td class="num">'+fmt(rev.company.contract_amount)+'</td></tr></tfoot></table>';
